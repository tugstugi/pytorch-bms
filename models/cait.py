...
class CaitEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        cls_tokens = self.e.cls_token.expand(B, -1, -1)

        x = x + self.e.pos_embed
        x = self.e.pos_drop(x)

        for i, blk in enumerate(self.e.blocks):
            x = blk(x)

        for i, blk in enumerate(self.e.blocks_token_only):
...
class Cait(nn.Module):
    def __init__(self, backbone, args):
        super().__init__()
        self.encoder = CaitEncoder(backbone, args)
        self.decoder = CaitDecoder(args)
